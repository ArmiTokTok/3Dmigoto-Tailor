...
# TODO add support to blender 3.5
# TODO add export all vb file automatically.
# TODO add support Normal are 4D problem.
#  增加一键导出所有body_part

# Don't use this script, it is broken now.
    ...
    if blend_indices:
        ...
        for vertex in mesh.vertices:
            for semantic_index in sorted(blend_indices.keys()):
                for i, w in zip(blend_indices[semantic_index][vertex.ib_index], blend_weights[semantic_index][vertex.ib_index]):
                    if w == 0.0:
                        continue
                    obj.vertex_groups[i].add((vertex.ib_index,), w, 'REPLACE')
def import_uv_layers(mesh, obj, texcoords, flip_texcoord_v):
    for (texcoord, data) in sorted(texcoords.items()):
        ...
        for components in components_list:
            ...
            for l in mesh.loops:
                blender_uvs.data[l.ib_index].uv = flip_uv(uvs[l.vertex_index])
        ...
        for component in range(dim):
            ...
            if type(data[0][0]) == int:
                ...
                for v in mesh.vertices:
                    ...
                    if val < 0x80000000:
                        layer.data[v.ib_index].value = val
                    else:
                        layer.data[v.ib_index].value = struct.unpack('i', struct.pack('I', val))[0]
            elif type(data[0][0]) == float:
                mesh.vertex_layers_float.new(name=layer_name)
                layer = mesh.vertex_layers_float[layer_name]
                for v in mesh.vertices:
                    layer.data[v.ib_index].value = data[v.ib_index][component]
            else:
    ...
    for elem in vb.layout:
        ...
        if translated_elem_name == 'POSITION':
            ...
            if len(data[0]) <= 3 or vertex_color_layer_channels == 4:
                ...
                for l in mesh.loops:
                    color_layer[l.ib_index].color = list(data[l.vertex_index]) + [0] * (c - len(data[l.vertex_index]))
            else:
                ...
                for l in mesh.loops:
                    color_layer[l.ib_index].color = data[l.vertex_index][:3]
                    alpha_layer[l.ib_index].color = [data[l.vertex_index][3], 0, 0]
    ...
    for elem in layout:
        ...
        if elem.name == 'POSITION':
            ...
            if elem.name in mesh.vertex_colors:
                vertex[elem.name] = elem.clip(list(mesh.vertex_colors[elem.name].data[blender_loop_vertex.ib_index].color))
            else:
                vertex[elem.name] = list(mesh.vertex_colors[elem.name+'.RGB'].data[blender_loop_vertex.ib_index].color)[:3] + \
                                    [mesh.vertex_colors[elem.name+'.A'].data[blender_loop_vertex.ib_index].color[0]]
            ...
            for uv_name in ('%s.xy' % elem.name, '%s.zw' % elem.name):
                if uv_name in texcoords:
                    uvs += list(texcoords[uv_name][blender_loop_vertex.ib_index])
...
def export_3dmigoto(operator, context, vb_path, ib_path, fmt_path):
    ...
    for uv_layer in mesh.uv_layers:
        ...
        for l in mesh.loops:
            uv = flip_uv(uv_layer.data[l.ib_index].uv)
            texcoords[l.ib_index] = uv
