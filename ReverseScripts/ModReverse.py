...
if __name__ == "__main__":
    ...
    for ib_num in range(len(ib_file_list)):
        ...
        while i < len(tmp_ib_bytearray):
            tmp_byte = struct.pack("i", struct.unpack("I", tmp_ib_bytearray[i:i+pack_stride])[0])
    ...
    for category in category_offset_dict:
        ...
        # TODO 只有offset = 0 的能直接复制过去，其他的都要减去offset
        ...
        if offset == 0 :
            ...
            while i < len(ib_file_bytearray):
                tmp_byte = struct.pack("i", struct.unpack("I",ib_file_bytearray[i:i+4])[0])
                int_num = int.from_bytes(tmp_byte, "little")
                real_num = int(int_num - offset/stride)
                # print(real_num)


                real_byte = int.to_bytes(real_num, signed=False, byteorder="little",length=4)
                # print(tmp_byte)
                # print(real_byte)
